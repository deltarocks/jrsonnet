...
#[derive(Debug)]
struct Walked {
	/// All visited files keyed by `format!("{source}")` (canonical path string).
...
fn process(
	...
	// 	if import_kind != ImportKind::Bin && existing.raw_text.is_none() {
	// 		existing.raw_text = Some(
	// 			str::from_utf8(&existing.raw_bytes)
	// 				.map_err(|_| Error::NotUtf8(source.clone(), import_kind))?
	// 				.to_owned(),
	// 		);
	// 	};
	// 	match import_kind {
	// 		ImportKind::Normal => existing.access.normal = true,
	// 		ImportKind::Str => existing.access.str = true,
	// 		ImportKind::Bin => existing.access.bin = true,
	// 	}
	// 	return Ok(());
	// }
	...
	if data.raw_text.is_none() && import_kind != ImportKindKind::ImportbinKw {
		let raw_text = String::from_utf8(data.raw_bytes.clone())
			.map_err(|e| Error::NotUtf8(source.clone(), import_kind))?;
	...
	let imports = if import_kind == ImportKindKind::ImportKw && !data.access.normal {
		...
		collect_imports(code).map_err(|e| match e {
			ImportScanError::Parse(n) => Error::Parse {
				path: source.clone(),
			},
...
fn collect_imports(source: &str) -> Result<Vec<RawImport>, ImportScanError> {
	...
	if !errs.is_empty() {
		return Err(ImportScanError::Parse(errs.len()));
	}
	let mut out = Vec::new();
